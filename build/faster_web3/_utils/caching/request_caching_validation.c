...
{
    PyObject *tmp;
    if (!(tmp = PyImport_ImportModule("dc3d2f5c6be03b6a4493__mypyc"))) return NULL;
    PyObject *capsule = PyObject_GetAttrString(tmp, "init_faster_web3____utils___caching___request_caching_validation");
    Py_DECREF(tmp);
    if (capsule == NULL) return NULL;
    void *init_func = PyCapsule_GetPointer(capsule, "dc3d2f5c6be03b6a4493__mypyc.init_faster_web3____utils___caching___request_caching_validation");
    Py_DECREF(capsule);
    if (!init_func) {
        return NULL;
    }
