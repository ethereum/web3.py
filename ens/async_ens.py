...
class AsyncENS(BaseENS):
    ...
    async def reverser(self,
                       target_address: ChecksumAddress) -> Optional['AsyncContract']:
    ...
    async def setup_address(
        ...
        resolver: 'AsyncContract' = await self._set_resolver(name,
                                                             transact=transact)
        return await resolver.functions.setAddr(raw_name_to_hash(name),  # type: ignore
                                                address).transact(transact)
    ...
    async def set_text(
        ...
        if r:
            if await _async_resolver_supports_interface(r, GET_TEXT_INTERFACE_ID):
                return await r.functions.setText(node, key,  # type: ignore
                                                 value).transact(transaction_dict)
            else:
                raise UnsupportedFunction(
                    f"Resolver for name `{name}` does not support `text` function"
                )
    ...
    async def _resolve(self, name: str,
                       fn_name: str = 'addr') -> Optional[Union[ChecksumAddress, str]]:
        normal_name = normalize_name(name)
    ...
    async def _setup_reverse(
        ...
        revers_registrar = await self._reverse_registrar()
        return await revers_registrar.functions.setName(  # type: ignore
    ...
    async def _assert_control(self, account: ChecksumAddress, name: str,
...
async def _async_resolver_supports_interface(resolver: 'AsyncContract',
