...
class AsyncENS(BaseENS):
    ...
    async def name(self, address: ChecksumAddress) -> Awaitable[Optional[str]]:
        ...
        """
        reversed_domain = address_to_reverse_domain(address)
        return await self.resolve(reversed_domain, get='name')
    ...
    async def setup_address(
        ...
        if address is None:
            address = EMPTY_ADDR_HEX
        transact['from'] = owner
        resolver: 'Contract' = await self._set_resolver(name, transact=transact)
        return await resolver.functions.setAddr(raw_name_to_hash(name), address).transact(transact)
    ...
    async def resolve(self, name: str,
        ...
        if resolver:
            lookup_function = getattr(resolver.functions, get)
            namehash = normal_name_to_hash(normal_name)
            address = await lookup_function(namehash).call()
            if is_none_or_zero_address(address):
                return None
            return address
        else:
    ...
    async def _set_resolver(
        ...
        return await self._resolverContract(address=resolver_addr)
    ...
    async def _setup_reverse(
        ...
        if name:
            ...
            name = ''
        transact['from'] = address
        return await self._reverse_registrar().functions.setName(name).transact(transact)
