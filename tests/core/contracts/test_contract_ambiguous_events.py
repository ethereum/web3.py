...
def test_ambiguous_events_reference_errors(
    ambiguous_event_contract: "Contract",
):
    message = "Attempted to find the event 'LogSingleArg' but more than one was found."
    with pytest.raises(
        MismatchedABI,
        match=message,
    ):
        ambiguous_event_contract.events.LogSingleArg.abi
    ...
    with pytest.raises(
        ...
        ambiguous_event_contract.events.LogSingleArg(1).abi
...
def test_async_ambiguous_events_reference_errors(
    async_ambiguous_event_contract: "AsyncContract",
) -> None:
    message = "Attempted to find the event 'LogSingleArg' but more than one was found."
    with pytest.raises(
        ...
        async_ambiguous_event_contract.events["LogSingleArg"]
    ...
    with pytest.raises(
        ...
        async_ambiguous_event_contract.events.LogSingleArg(1)
