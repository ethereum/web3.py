...
def test_throws_error_if_block_out_of_range(web3, math_contract):
    sync(web3.providers[0].make_request(method='evm_mine', params=[20]))
...
def test_accepts_latest_block(web3, math_contract):
    sync(web3.providers[0].make_request(method='evm_mine', params=[5]))
...
def test_accepts_block_hash_as_identifier(web3, math_contract):
    blocks = sync(web3.providers[0].make_request(method='evm_mine', params=[5]))
    math_contract.functions.increment().transact()
    more_blocks = sync(web3.providers[0].make_request(method='evm_mine', params=[5]))
