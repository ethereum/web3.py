...
async def test_async_contract_constructor_transact_with_constructor_with_arguments(
    ...
    assert expected_a == await AsyncWithConstructorArgumentsContract(
        address=contract_address).functions.data_a().call()
    assert expected_b == await AsyncWithConstructorArgumentsContract(
        address=contract_address).functions.data_b().call()
...
async def test_async_contract_constructor_transact_with_constructor_with_address_arguments(
        ...
        WITH_CONSTRUCTOR_ADDRESS_RUNTIME,
        address_conversion_func):
    deploy_txn = await AsyncWithConstructorAddressArgumentsContract.constructor(TEST_ADDRESS).transact()  # noqa: E501
    txn_receipt = await async_w3.eth.wait_for_transaction_receipt(deploy_txn)
    assert txn_receipt is not None
    assert txn_receipt['contractAddress']
    contract_address = address_conversion_func(txn_receipt['contractAddress'])
    blockchain_code = await async_w3.eth.get_code(contract_address)
    assert blockchain_code == decode_hex(WITH_CONSTRUCTOR_ADDRESS_RUNTIME)
    assert TEST_ADDRESS == await AsyncWithConstructorAddressArgumentsContract(
        address=contract_address).functions.testAddr().call()


@pytest.mark.asyncio
async def test_async_contract_constructor_build_transaction_to_field_error(AsyncMathContract):
    with pytest.raises(ValueError):
