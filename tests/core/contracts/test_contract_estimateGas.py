...
async def test_async_estimate_gas_not_sending_ether_to_nonpayable_function(
        ...
        async_estimate_gas,
        async_transact):
    gas_estimate = await async_estimate_gas(contract=async_payable_tester_contract,
                                            contract_function='doNoValueCall')
...
async def test_async_estimate_gas_sending_ether_to_nonpayable_function(
        ...
        async_estimate_gas):
    with pytest.raises(ValidationError):
        await async_estimate_gas(contract=async_payable_tester_contract,
                                 contract_function='doNoValueCall',
                                 tx_params={'value': 1})
...
async def test_async_estimate_gas_block_identifier_unique_estimates(
        ...
        async_math_contract,
        async_transact):
    txn_hash = await async_transact(contract=async_math_contract, contract_function="increment")
