...
@pytest.fixture
def async_tester_provider():
    return AsyncEthereumTesterProvider()
...
@pytest.fixture
def async_eth_tester(async_tester_provider):
    return async_tester_provider.ethereum_tester
...
@pytest.fixture
def async_w3(async_tester_provider):
    return Web3(async_tester_provider)
...
@pytest.fixture
async def async_foo_contract(async_eth_tester, async_w3):
    # For simplicity of this example we statically define the
    # contract code here. You might read your contracts from a
    # file, or something else to test with in your own code
    #
    ...
    async_eth_tester_accounts = await async_eth_tester.get_accounts()
    ...
    print(f'{tx_receipt=}')
    # instantiate and return an instance of our contract.
    return await FooContract(tx_receipt.contractAddress)
...
async def test_async_initial_greeting(async_foo_contract):
    contract = await async_foo_contract
    hw = contract.caller.bar()
    assert hw == "hello world"
