...
def test_transacting_with_contract_with_arguments(w3,
                                                  math_contract,
                                                  transact,
                                                  call,
                                                  transact_args,
...
def test_deploy_when_default_account_is_set(w3,
...
def test_transact_when_default_account_is_set(w3,
                                              wait_for_transaction,
                                              math_contract,
                                              transact):
    w3.eth.default_account = w3.eth.accounts[1]
...
def test_transacting_with_contract_with_string_argument(w3, string_contract, transact, call):
...
def test_transacting_with_contract_with_bytes32_array_argument(w3,
                                                               arrays_contract,
                                                               transact,
                                                               call):
...
def test_transacting_with_contract_with_byte_array_argument(w3, arrays_contract, transact, call):
...
async def test_async_transacting_with_contract_no_arguments(
        ...
        async_transact,
        async_call):
    initial_value = await async_call(contract=async_math_contract,
                                     contract_function='counter')
    ...
    final_value = await async_call(contract=async_math_contract,
                                   contract_function='counter')
...
async def test_async_transact_not_sending_ether_to_nonpayable_function(
        async_w3,
        async_payable_tester_contract,
        async_transact,
        async_call):
    initial_value = await async_call(contract=async_payable_tester_contract,
                                     contract_function='wasCalled')

    assert initial_value is False
    txn_hash = await async_transact(contract=async_payable_tester_contract,
                                    contract_function='doNoValueCall')
    txn_receipt = await async_w3.eth.wait_for_transaction_receipt(txn_hash)
    assert txn_receipt is not None

    final_value = await async_call(contract=async_payable_tester_contract,
                                   contract_function='wasCalled')
...
async def test_async_transacting_with_contract_with_string_argument(
        async_w3,
        async_string_contract,
        async_transact,
        async_call):
    # eth_abi will pass as raw bytes, no encoding
    # unless we encode ourselves
    txn_hash = await async_transact(contract=async_string_contract,
                                    contract_function='setValue',
                                    func_args=["ÄLÄMÖLÖ".encode('utf8')])
    txn_receipt = await async_w3.eth.wait_for_transaction_receipt(txn_hash)
    assert txn_receipt is not None

    final_value = await async_call(contract=async_string_contract,
                                   contract_function='getValue')
...
async def test_async_transacting_with_contract_with_bytes32_array_argument(
    ...
    txn_hash = await async_transact(contract=async_arrays_contract,
                                    contract_function="setBytes32Value",
                                    func_args=[new_bytes32_array])
    txn_receipt = await async_w3.eth.wait_for_transaction_receipt(txn_hash)
    assert txn_receipt is not None

    final_value = await async_call(contract=async_arrays_contract,
                                   contract_function="getBytes32Value")
    assert final_value == new_bytes32_array
...
async def test_async_transacting_with_contract_respects_explicit_gas(
    ...
    final_value = await async_call(contract=string_contract,
                                   contract_function='getValue')
    assert to_bytes(text=final_value) == to_bytes(text="ÄLÄMÖLÖ")
...
async def test_async_auto_gas_computation_when_transacting(
    ...
    gas_estimate = await string_contract.functions.setValue(to_bytes(text="ÄLÄMÖLÖ")).estimate_gas()
...
async def test_async_fallback_transacting_with_contract(
        ...
        async_call):
    initial_value = await async_call(contract=async_fallback_function_contract,
                                     contract_function='getData')
    txn_hash = await async_fallback_function_contract.fallback.transact()
    txn_receipt = await async_w3.eth.wait_for_transaction_receipt(txn_hash)
    assert txn_receipt is not None

    final_value = await async_call(contract=async_fallback_function_contract,
                                   contract_function='getData')
...
async def test_async_receive_function(async_receive_function_contract, async_call):
    initial_value = await async_call(contract=async_receive_function_contract,
                                     contract_function='getText')
    assert initial_value == ''
    await async_receive_function_contract.receive.transact()
    final_value = await async_call(contract=async_receive_function_contract,
                                   contract_function='getText')
    assert final_value == 'receive'
