...
def test_miner_set_extra(web3_empty, wait_for_block):
    ...
    initial_extra = decode_hex(web3.eth.getBlock(web3.eth.block_number)['extraData'])
    ...
    with Timeout(60) as timeout:
        while True:
            extra_data = decode_hex(web3.eth.getBlock(web3.eth.block_number)['extraData'])
            if extra_data == new_extra_data:
                break
            timeout.sleep(random.random())

    after_extra = decode_hex(web3.eth.getBlock(web3.eth.block_number)['extraData'])
