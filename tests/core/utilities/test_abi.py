...
@pytest.mark.parametrize(
    "name,args,kwargs,expected_selector,expected_arguments",
    [
...
def test_get_abi_element_info(
    ...
    function_info = get_abi_element_info(contract_abi, name, *args, **kwargs)
    assert function_info["abi"] == get_abi_element(contract_abi, name, *args, **kwargs)
...
@pytest.mark.parametrize(
    "abi,function_identifier,args,kwargs,expected_abi",
    (
        (
            CONTRACT_ABI,
            "setValue",
...
@pytest.mark.parametrize(
    "abi,name,args,kwargs,expected_error,expected_message",
    (
        (
            CONTRACT_ABI,
            1,
...
def test_get_abi_element_raises_with_invalid_parameters(
    ...
    with pytest.raises(expected_error, match=expected_message):
        get_abi_element(abi, name, *args, **kwargs)  # type: ignore
...
def test_get_constructor_function_abi_raises_if_not_found(contract_abi: ABI) -> None:
    with pytest.raises(
        ABIConstructorNotFound,
        match="No constructor function was found in the contract ABI.",
    ):
...
def test_get_receive_function_abi_raises_if_not_found(contract_abi: ABI) -> None:
    with pytest.raises(
        ABIReceiveNotFound, match="No receive function was found in the contract ABI."
    ):
...
def test_get_fallback_function_abi(abi: ABI, expected_abi: ABIElement) -> None:
    fallback_abi = get_fallback_function_abi(abi)
    assert fallback_abi == expected_abi


def test_get_fallback_function_abi_raises_if_not_found(contract_abi: ABI) -> None:
    with pytest.raises(
        ABIFallbackNotFound, match="No fallback function was found in the contract ABI."
    ):
