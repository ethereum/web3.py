...
def test_toBytes_is_deprecated():
    with pytest.warns(
        DeprecationWarning, match="toBytes is deprecated in favor of to_bytes"
    ):
...
def test_toText_is_deprecated():
    with pytest.warns(
        DeprecationWarning, match="toText is deprecated in favor of to_text"
    ):
        assert Web3.toText("0x5765697264") == "Weird"
...
def test_toInt_is_deprecated():
    with pytest.warns(
        DeprecationWarning, match="toInt is deprecated in favor of to_int"
    ):
        assert Web3.toInt(True) == 1
...
def test_toHex_is_deprecated():
    with pytest.warns(
        DeprecationWarning, match="toHex is deprecated in favor of to_hex"
    ):
...
def test_toJSON_is_deprecated():
    with pytest.warns(
        DeprecationWarning, match="toJSON is deprecated in favor of to_json"
    ):
        assert Web3.toJSON(AttributeDict({"a": 1})) == '{"a": 1}'


def test_isAddress_is_deprecated():
    with pytest.warns(
        DeprecationWarning, match="isAddress is deprecated in favor of is_address"
    ):
        assert Web3.isAddress("snek") is False
...
def test_isChecksumAddress_is_deprecated():
    with pytest.warns(
        ...
        assert Web3.isChecksumAddress("snek") is False
...
def test_toWei_is_deprecated():
    with pytest.warns(
        DeprecationWarning, match="toWei is deprecated in favor of to_wei"
    ):
        assert Web3.toWei(1, "gwei") == 1000000000


def test_fromWei_is_deprecated():
    with pytest.warns(
        DeprecationWarning, match="fromWei is deprecated in favor of from_wei"
    ):
        assert Web3.fromWei(1000000000, "gwei") == 1
