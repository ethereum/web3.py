...
def async_pubilc_resolver_factory(async_w3):
    return async_w3.eth.contract(
        bytecode=resolver_bytecode,
        bytecode_runtime=resolver_bytecode_runtime,
        abi=resolver_abi,
...
# add module scope with above module-scoped `event_loop` for better performance
@pytest_asyncio.fixture(scope="module")
async def async_ens_setup(async_w3):
    ...
    # create public resolver
    public_resolver = await async_deploy(async_w3, async_pubilc_resolver_factory,
                                         ens_key, args=[ens_contract.address])
    ...
    # create reverse resolver
    reverse_resolver = await async_deploy(async_w3, async_default_reverse_resolver,
                                          ens_key, args=[ens_contract.address])
    ...
    await ens_contract.functions.setSubnodeOwner(
        b'\0' * 32,
        async_w3.keccak(text='reverse'),
        ens_key
    ).transact({'from': ens_key})

    await ens_contract.functions.setSubnodeOwner(
        reverse_tld_namehash,
        async_w3.keccak(text='addr'),
        ens_key
    ).transact({'from': ens_key})
    ...
    # create simple resolver
    simple_resolver = await async_deploy(async_w3, async_simple_resolver,
                                         ens_key, args=[ens_contract.address])

    # set owner of simple-resolver.eth to an account controlled by tests
    await ens_contract.functions.setSubnodeOwner(
        eth_namehash,
        async_w3.keccak(text='simple-resolver'),
        second_account
    ).transact({'from': ens_key})
    ...
    # create extended resolver
    extended_resolver = await async_deploy(async_w3, async_extended_resolver,
                                           ens_key, args=[ens_contract.address])

    # set owner of extended-resolver.eth to an account controlled by tests
    await ens_contract.functions.setSubnodeOwner(
        eth_namehash,
        async_w3.keccak(text='extended-resolver'),
        second_account
    ).transact({'from': ens_key})
    ...
    return await AsyncENSFactory.fromWeb3(async_w3, ens_contract.address)
