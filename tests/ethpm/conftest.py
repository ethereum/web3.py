...
@pytest.fixture
def escrow_manifest_v3(get_manifest_v3):
    escrow_manifest = get_manifest_v3("escrow")
...
@pytest.fixture
def get_factory_v3(get_manifest_v3, escrow_manifest_v3, w3):
    def _get_factory_v3(package, factory_name):
        manifest = get_manifest_v3(package)
        # Special case to fetch escrow manifest with added deployment bytecode
        if package == "escrow":
            manifest = escrow_manifest_v3
        Pkg = Package(manifest, w3)
        factory = Pkg.get_contract_factory(factory_name)
        return factory

    return _get_factory_v3
...
@pytest.fixture
def manifest_with_empty_deployments(tmpdir, safe_math_manifest_v3):
    manifest = copy.deepcopy(safe_math_manifest_v3)
...
@pytest.fixture
def escrow_package_v3(deployer, w3):
...
@pytest.fixture
def safe_math_lib_package_with_alias_v3(deployer, w3):
...
@pytest.fixture
def manifest_with_no_matching_deployments(w3, tmpdir, safe_math_manifest_v3):
    w3.testing.mine(5)
...
@pytest.fixture
def manifest_with_multiple_matches(w3, tmpdir, safe_math_manifest_v3):
    w3.testing.mine(5)
