...
class GoEthereumEthModuleTest(EthModuleTest):
    ...
    def test_eth_sign_typed_data(self, w3, test_sender_account_dual_type):
        super().test_eth_sign_typed_data(w3, test_sender_account_dual_type)

    @pytest.mark.xfail(reason="eth_signTypedData has not been released in geth")
    def test_invalid_eth_sign_typed_data(self, w3, test_sender_account_dual_type):
        super().test_invalid_eth_sign_typed_data(w3, test_sender_account_dual_type)
    ...
    def test_eth_estimate_gas(
        ...
        super().test_eth_estimate_gas(w3, test_sender_account_dual_type)

    @pytest.mark.xfail(reason="Inconsistently creating timeout issues.", strict=False)
    def test_eth_estimate_gas_with_block(
        self, w3: "Web3", test_sender_account_dual_type: ChecksumAddress
    ) -> None:
        super().test_eth_estimate_gas_with_block(w3, test_sender_account_dual_type)
    ...
    def test_eth_get_raw_transaction_by_block(
        ...
        super().test_eth_get_raw_transaction_by_block(
            w3, test_sender_account_dual_type, block_with_txn
        )
...
class GoEthereumAsyncEthModuleTest(AsyncEthModuleTest):
    ...
    async def test_eth_sign_typed_data(
        self, async_w3, test_sender_account_dual_type, async_skip_if_testrpc
    ):
        await super().test_eth_sign_typed_data(
            async_w3, test_sender_account_dual_type, async_skip_if_testrpc
        )
