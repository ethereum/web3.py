...
class ContractEvent(BaseContractEvent):
    ...
    def get_logs(
        ...
        all_event_logs = tuple(
            parse_transaction_for_event(event_abi, entry) for entry in logs
        )
    ...
    def create_filter(
        ...
        *,  # PEP 3102
        ...
        log_filter.log_entry_formatter = parse_transaction_for_event(
            self._get_event_abi()
        )
    ...
    def build_filter(self) -> EventFilterBuilder:
        builder = EventFilterBuilder(
            self._get_event_abi(),
            self.w3.codec,
            formatter=parse_transaction_for_event(self._get_event_abi()),
        )
