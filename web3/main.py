...
class Web3:
    ...
    RankingRequestManager = RankingRequestManager
    ...
    # NOTE:  might be ok to just work off the provider_strategy param if we're sure
    # we don't add more. seems easier to introduce two new optional params than
    # having to have people change their code down the timeline. pita, imo. big pita.
    def __init__(self, providers=empty, middlewares=None, modules=None, ens=empty,
                 provider_type='default', provider_strategy='default'):
        if provider_type == 'default':
            self.manager = RequestManager(self, providers, middlewares)
        elif provider_type == 'ranking_provider':
            self.manager = RankingRequestManager(self, providers, middlewares, provider_strategy)
        else:
            msg = 'The provider_type {} you specified is invalid.'.format(provider_type)
